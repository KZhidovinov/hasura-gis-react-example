...
DirectSelect.fireUpdate = function (state) {
    this.map.fire(Constants.events.UPDATE, {
        ...
        coordPaths: state.selectedCoordPaths
    });
};
...
DirectSelect.onMidpoint = function (state, e) {
    ...
    // add new point to ids list
    state.feature.properties.node_ids.splice(parseInt(about.coord_path), 0, null)
...
DirectSelect.onTrash = function (state) {
    state.selectedCoordPaths.sort().reverse().forEach(id => {
        state.feature.removeCoordinate(id)
        // remove point from ids list
        state.feature.properties.node_ids.splice(parseInt(id), 1)
    });
    this.fireUpdate(state);
    state.selectedCoordPaths = [];
    this.clearSelectedCoordinates();
